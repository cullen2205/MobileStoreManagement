...
using MobileStore.LoginService;
        ...
        public ActionResult Register(user obj, HttpPostedFileBase file)
        {
            if (ModelState.IsValid)
            {
                if (file != null && file.ContentLength > 0)
                {
                    ...
                    var path = Path.Combine(Server.MapPath("~/App_Start/Upload/Users"), Hash.EncMD5(filename) + _ex);
                    file.SaveAs(path);

                    var pwd = Hash.EncMD5(obj.pwd);
                    // them nguoi dung
                    addSV.Add_user(addSV.ID_Return("users"), obj.username, obj.fullname, pwd, obj.email, obj.address, 
                        obj.phone, "~/App_Start/Upload/Users/" + Hash.EncMD5(filename) + _ex, 1);
