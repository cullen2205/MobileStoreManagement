using MyWCFService.Models.MyEditModels;
using System;
    ...
    public class ViewService : IViewService
    {
        ...
        public order[] Get_order(string utoken, int id, int? user_id = null, int? admin_id = null)
        {
            try
            {
                ...
                else if (db.tokens.Where(m => m.value.Equals(utoken) && m.admin_id.Equals(admin_id)) != null)
                {
                    if (id == 0)
            ...
            public static T[] GetData(string nametable, int id)
            {
                try
                {
                    switch (nametable)
                    {
                        ...
                        case "product":
                            if (id == 0)
                                return db.products.Cast<T>().ToArray();
                            else if (id > 0)
